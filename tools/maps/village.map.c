...
void StaticSprite(struct s_map *map, struct s_lib_sprite *libsprite, struct s_lib_event *libevent)
{
     ...
     libsprite->sprites[1].cur_anim=ANIMATION_DROITE;
...
{
     GET_MAPEVENT(88,2,map,libevent).type=EVENT_TELEPORT;
     GET_MAPEVENT(88,2,map,libevent).onaction=EVENT_ACTION_AUTO;
     GET_MAPEVENT(88,2,map,libevent).proba=100;
     GET_MAPEVENT(88,2,map,libevent).player_anim=-1;
     GET_MAPEVENT(88,2,map,libevent).sound=FSOUND_Sample_Load(FSOUND_FREE,EVENT_SON,FSOUND_NORMAL,0,0);
     GET_MAPEVENT(88,2,map,libevent).is_unique=0;
     GET_MAPEVENT(88,2,map,libevent).next=NULL;
     GET_MAPEVENT(88,2,map,libevent).param=calloc(1,sizeof(struct s_event_param_teleport));
     ((struct s_event_param_teleport*)GET_MAPEVENT(88,2,map,libevent).param)->filename=(char*)strdup("map/maison1");
     ((struct s_event_param_teleport*)GET_MAPEVENT(88,2,map,libevent).param)->startX=6*32;
     ((struct s_event_param_teleport*)GET_MAPEVENT(88,2,map,libevent).param)->startY=11*32;

     GET_MAPEVENT(89,8,map,libevent).type=EVENT_TELEPORT;
     GET_MAPEVENT(89,8,map,libevent).onaction=EVENT_ACTION_AUTO;
     GET_MAPEVENT(89,8,map,libevent).proba=100;
     GET_MAPEVENT(89,8,map,libevent).player_anim=-1;
     GET_MAPEVENT(89,8,map,libevent).sound=FSOUND_Sample_Load(FSOUND_FREE,EVENT_SON,FSOUND_NORMAL,0,0);
     GET_MAPEVENT(89,8,map,libevent).is_unique=0;
     GET_MAPEVENT(89,8,map,libevent).next=NULL;
     GET_MAPEVENT(89,8,map,libevent).param=calloc(1,sizeof(struct s_event_param_teleport));
     ((struct s_event_param_teleport*)GET_MAPEVENT(89,8,map,libevent).param)->filename=(char*)strdup("map/maison2_1");
     ((struct s_event_param_teleport*)GET_MAPEVENT(89,8,map,libevent).param)->startX=5*32;
     ((struct s_event_param_teleport*)GET_MAPEVENT(89,8,map,libevent).param)->startY=7*32;

     GET_MAPEVENT(99,4,map,libevent).type=EVENT_TELEPORT;
     GET_MAPEVENT(99,4,map,libevent).onaction=EVENT_ACTION_AUTO;
     GET_MAPEVENT(99,4,map,libevent).proba=100;
     GET_MAPEVENT(99,4,map,libevent).player_anim=-1;
     GET_MAPEVENT(99,4,map,libevent).sound=FSOUND_Sample_Load(FSOUND_FREE,EVENT_SON,FSOUND_NORMAL,0,0);
     GET_MAPEVENT(99,4,map,libevent).is_unique=0;
     GET_MAPEVENT(99,4,map,libevent).next=NULL;
     GET_MAPEVENT(99,4,map,libevent).param=calloc(1,sizeof(struct s_event_param_teleport));
     ((struct s_event_param_teleport*)GET_MAPEVENT(99,4,map,libevent).param)->filename=(char*)strdup("map/01");
     ((struct s_event_param_teleport*)GET_MAPEVENT(99,4,map,libevent).param)->startX=32;
     ((struct s_event_param_teleport*)GET_MAPEVENT(99,4,map,libevent).param)->startY=128;
     ...
     ((struct s_event_param_dialog*)libevent->sprite_event[1].param)->dialog=(char*)strdup("Mon livre est tombé!\nComment vais-je faire?");
}
